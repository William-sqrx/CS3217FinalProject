...
struct Grid {
    ...
    func getNodeSize(numTileX: Int = 1, numTileY: Int = 1) -> CGSize {
        assert(1 <= numTileX && numTileX < numCols)
        assert(1 <= numTileY && numTileY < numLanes)
    ...
    init(width: Double, height: Double, numCols: Int, numLanes: Int, laneSpacing: Double) {
        self.width = width
        self.height = height
